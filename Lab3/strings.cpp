...
void strings::loadStrings(const std::vector<std::pair<TKEY, UINT>> keyValuePairs)
{
	for(auto iter = keyValuePairs.cbegin();  iter != keyValuePairs.cend(); ++iter)
	{
		loadString((*iter).second, (*iter).first);
	}
}
